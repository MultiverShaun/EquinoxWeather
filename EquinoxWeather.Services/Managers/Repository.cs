...
namespace EquinoxWeather.Services.Managers
{
	public class Repository : IRepository
	{
		...
		private const long CacheTtlMs = 10 * 60 * 1000;

		public Repository(HttpClient http, ILocalStorageService localStorage)
		{
			_http = http;
		...
		{
			var cacheKey = $"cache_weather_{latitude}_{longitude}_{(unitTempIsF ? "f" : "c")}_{(unitDistanceIsMph ? "mph" : "kmh")}";
			var cached = await TryGetCache<OpenMeteoResponse>(cacheKey);
			...
			var weatherData = JsonSerializer.Deserialize<OpenMeteoResponse>(content, JsonOptions)
		...
		{
			var cacheKey = $"cache_aqi_{latitude}_{longitude}";
			var cached = await TryGetCache<AirQualityIndex>(cacheKey);
			if (cached is not null) return cached;
			...
			var content = await response.Content.ReadAsStringAsync();
			var airQuality = JsonSerializer.Deserialize<AirQualityIndex>(content, JsonOptions)
				?? throw new Exception("Could not deserialize AQI data.");

			await SetCache(cacheKey, airQuality);
			return airQuality;
		}
		...
		public async Task<GeocodingResponse> GetCitySuggestions(string input)
		{
			...
			var content = await response.Content.ReadAsStringAsync();
			return JsonSerializer.Deserialize<GeocodingResponse>(content, JsonOptions)
		...
		private async Task<T?> TryGetCache<T>(string key)
		{
			try
			{
				...
				var age = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - entry.CachedAt;
				return age < CacheTtlMs ? entry.Data : default;
			}
			catch
			{
				return default;
		...
		private async Task SetCache<T>(string key, T data)
		{
			try
			{
				await _localStorage.SetItemAsync(key, new CacheEntry<T>(data, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()));
			}
			catch { }
		}

		private record CacheEntry<T>(T Data, long CachedAt);
	}
}
